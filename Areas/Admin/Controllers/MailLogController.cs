...
namespace LSDCS.Web.Areas.Admin.Controllers
{
    ...
    public class MailLogController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var mailLog = await _mailLogService.GetAllMailLogWithClientMatterRecipientNoneDeletedAsync();
            return View(mailLog);
        }
        ...
        public async Task<IActionResult> DeletedAllMailLogsList()
        {



            var milLog = await _mailLogService.GetAllDeletedAllMailLogAsync();
            return View(milLog);
        ...
        [HttpPost]
        [Authorize(Roles = $"{RoleConsts.Admin}")]
        public async Task<IActionResult> Add(MailLogAddDto mailLogAddDto, string ALICI_TO, string ALICI_CC)
        {
        ...
        public async Task<IActionResult> MailLogUpdate(MailLogUpdateDto mailLogUpdateDto, string ALICI_TO, string ALICI_CC)
        {
            ...
            if (result.IsValid)
            {
                ...
                return RedirectToAction("MailLogDetails", "MailLog", new { Area = "Admin", mailLogId = mailLogUpdateDto.Id }); ;
            }
            else
            {
                var clients = await _clientService.GetAllClientsNoneDelete();
